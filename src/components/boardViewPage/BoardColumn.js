...
const BoardColumn = ({ boardInfo, setShowModal, setSelectedBoard, newCards, setNewCards, setNewBoard }) => {
    ...
    return ( <
        ...
        <
        img src = { threeDots }
        />  { showMore ? < button className = 'deleteSectionButton'
            onClick = { handleDeleteSection } > Delete Section < /button> : <></ > } <
        /button> <
        /div> <
        /div> <
        div className = 'headerBottomRow' >
        <
        p className = 'addCards'
        onClick = { handleAddCardsClick } > Add more cards + < /p> { toBeRemovedCards.length > 0 && < button className = 'removeCardsButton'
            onClick = { handleCardDeletions } > Remove < img src = { trashIcon }
            /></button > } <
        /div> <
        /div> <
